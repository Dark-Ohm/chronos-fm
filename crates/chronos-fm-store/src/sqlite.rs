...
// callback reads. A chronos-fm process opens a single metadata store, so this global state is
...
fn profile_callback(sql: &str, duration: Duration) {
    ...
    if slow_threshold > 0 && elapsed_ms >= slow_threshold {
        tracing::warn!(target: "chronos_fm_store::sql", elapsed_ms, sql, "slow query");
    } else if LOG_ALL_QUERIES.load(Ordering::Relaxed) {
        tracing::debug!(target: "chronos_fm_store::sql", elapsed_ms, sql, "query");
    }
}
