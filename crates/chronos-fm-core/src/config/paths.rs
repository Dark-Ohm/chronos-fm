//! XDG-style path resolution for chronos-fm data locations.
//!
...
/// The chronos-fm config directory (`.../chronos-fm`).
pub fn config_dir() -> PathBuf {
    config_home().join(APP_DIR)
}

/// The chronos-fm data directory (`.../chronos-fm`).
pub fn data_dir() -> PathBuf {
    data_home().join(APP_DIR)
}

/// The chronos-fm cache directory (`.../chronos-fm`).
...
mod tests {
    ...
    fn config_file_lives_under_config_home() {
        ...
        assert!(file.ends_with("chronos-fm/config.toml"));
    }
    ...
    fn absolute_xdg_var_is_honoured() {
        ...
        assert_eq!(dir, PathBuf::from("/tmp/xdg-test-abs/chronos-fm"));
    }
    ...
    fn app_dirs_append_app_name_and_config_file_is_under_config_dir() {
        ...
        let _guard = env_lock();
        assert!(config_dir().ends_with("chronos-fm"));
        assert!(data_dir().ends_with("chronos-fm"));
        assert!(cache_dir().ends_with("chronos-fm"));
