# Contributing to Chronos-FM

Thanks for your interest in contributing! Chronos-FM is **pre-alpha**, so things move
...
- Snapshot tests use [`insta`](https://insta.rs) (e.g. the config parser in
  `chronos-fm-core`); refresh intentional changes with `cargo insta review`. Reach for
...
gpui-backed crates (`chronos-fm-ui`, `chronos-fm-pages`) enable gpui's `test-support`
...
CI measures coverage on two tiers: a Linux **core** tier (`-p chronos-fm-core`) and a
...
- `chronos-fm-core` drops below **80%** lines (aggregate *and* per file), or
...
```sh
cargo llvm-cov -p chronos-fm-core --fail-under-lines 80 --fail-under-file-lines 80
