...
the gotchas in the doc (`pkill -x chronos-fm`, find window by PID, black-first-frame).
...
| GUI build | `RUSTFLAGS="-L $HOME/.local/devlibs" cargo build --features gui --bin chronos-fm` |
