...
本書は WIT (WebAssembly Interface Type) で表現するプラグイン API の表面を定めます。`chronos-fm:plugin@0.1.0` がプラグイン作者向けの正式 world です。
...
```wit
// crates/chronos-fm-plugin-host/wit/world.wit
package chronos-fm:plugin@0.1.0;
...
| 観点 | 仕様 |
|------|------|
| host → plugin | **sync** な公開 API。内部は `chronos-fm-plugin-host` の専用 tokio runtime で wasmtime-wasi (async) を `block_on` 駆動。host 側は `cx.background_spawn` 経由で呼び UI を block しない ([ADR 0004](./adr/0004-remove-tokio.md)) |
...
```text
crates/chronos-fm-plugin-host/wit/
├── world.wit
├── deps/
│   └── chronos-fm-types/        # 共通 record / variant
...
- AI agent 向け MCP server `@chronos-fm/mcp-plugin-dev` の `chronos-fm_wit_lookup(name)` で query 可能
