...
本書は chronos-fm の web 公開面 (`chronos-fm.app` + `chronos.fm`) の構成・ホスティング・コンテンツ運用を定めます。
...
| ドメイン | 役割 | 取得状況 |
|---------|------|---------|
| **`chronos-fm.app`** | 正式サイト (ランディング・ダウンロード・docs・blog・release・plugin store) | 取得済 |
| **`chronos.fm`** | 短縮 URL / SNS 共有用 / CLI/開発者向け導線。すべて 301 で `chronos-fm.app/<path>` にリダイレクト + 一部短縮スキーム | 取得済 |

### chronos.fm リダイレクト仕様 (Cloudflare Workers)

- `chronos.fm/<any>` → `chronos-fm.app/<any>` に **path 保持で 301**
- 短縮スキーム (将来):
  - `chronos.fm/p/<plugin-id>` → `chronos-fm.app/plugins/<plugin-id>`
  - `chronos.fm/r/<release>` → `chronos-fm.app/releases/<release>` (もしくは GitHub release)
- HTTPS 強制
...
| デプロイ | `main` ブランチ → 本番、PR → preview (`<branch>.chronos-fm-web.pages.dev`) |
...
R2 は他用途でも使用:
- `coverage.chronos-fm.app` (PR ごとの HTML カバレッジレポートを保管。詳細は [`docs/testing.md`](./testing.md))
...
**zed.dev を土台 (DNA)** とする。職人的・エディトリアルなトーンが chronos-fm (Rust 製のパワーユーザ向けツール) の製品性格に最も合う。残り 2 つはアクセントとして部分採用:
...
├── public/
└── workers/
    └── chronos-fm-redirect.ts     # chronos.fm リダイレクト Worker
```
...
| カテゴリ | 場所 | 役割 |
|---------|------|------|
| 開発者・コントリビュータ向け docs | `chronos-fm/docs/` (Rust 本体リポジトリ内) | アーキテクチャ、ADR、WIT spec、permission モデル |
...
| README 翻訳 | `chronos-fm/docs/README.ja.md` (維持) | リポジトリ訪問者向け |
...
ページ全体像 (上から下のスクロール、zed.dev のホーム構成を chronos-fm 流に):
...
2. **"Why chronos-fm?"** — 3-4 ポイントで差別化 (Launcher first-class / Explorer first-class / WASM plugins / Spotlight 非依存の検索。README の柱を流用)
...
5. **OSS 透明性 = 社会的証明の置換** (pre-alpha でユーザがいないため testimonials は作らない):
   - live GitHub シグナル (star 数・最近のコミット activity feed・contributors) — zed の activity feed の chronos-fm 版
   - **メーカーズノート** (「なぜ chronos-fm を作るのか」。zed の team-letter 相当、個人/初期プロジェクトの信頼構築)
...
- Install ボタン: `chronos-fm://install?source=user/repo` で deeplink
...
- **メーカーズノート** (landing と共有可。なぜ chronos-fm を作るのか)
...
```ts
// workers/chronos-fm-redirect.ts (擬似コード)
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname.startsWith("/p/")) {
      ...
      return Response.redirect(`https://chronos-fm.app/plugins/${id}${url.search}`, 301);
    }
    if (url.pathname.startsWith("/r/")) {
      const tag = url.pathname.slice(3);
      return Response.redirect(`https://chronos-fm.app/releases/${tag}${url.search}`, 301);
    }

    // path 保持リダイレクト
    return Response.redirect(`https://chronos-fm.app${url.pathname}${url.search}`, 301);
  },
};
```
