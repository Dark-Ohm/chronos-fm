...
P1 で web (chronos-fm.app + chronos.fm) を立ち上げるにあたり、ホスティングプラットフォームを決める必要がある。
...
- **chronos.fm → chronos-fm.app の path 保持リダイレクト** を Worker で実装する方針 (詳細は [`docs/web.md`](../web.md) §1)
...
| 用途 | サービス |
|------|---------|
| **静的 / SSR サイト** (chronos-fm.app) | Cloudflare Pages |
| **chronos.fm リダイレクト** | Cloudflare Workers |
| **カバレッジ HTML レポート** (`coverage.chronos-fm.app`) | R2 + Pages routing |
| **将来の画像 CDN** (blog screenshot / plugin icon) | R2 |
| **分析** | Cloudflare Web Analytics (cookie 不要) |
| **検索 (docs)** | Pagefind (静的、ホスティング非依存) |
| **PR preview** | Cloudflare Pages 標準機能 (`<branch>.chronos-fm-web.pages.dev`) |
...
- chronos.fm のリダイレクト Worker が 5 行で書ける
- カバレッジ HTML を R2 にアップ → `coverage.chronos-fm.app/pr/<n>/` で公開、追加サービス不要
