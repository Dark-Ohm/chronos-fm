...
```text
1. 起動: ユーザー操作 (chronos-fm CLI / Plugin Store ボタン / `chronos-fm://install?source=user/repo`)
2. config.toml に追加 or memory state 更新
3. chronos-fm-plugin-host が install task を起動:
   a. source 解析 → git clone or download
   b. plugin.toml 読み取り、schema 検証
   c. engine.chronos-fm_version の範囲チェック (適合しなければ reject)
   d. component.wasm を wit-bindgen で validate (world 互換性)
   e. permission prompt 表示、ユーザー同意取得
   f. $DATA/chronos-fm/plugins/<plugin-id>/ にコピー (sha256 で integrity check)
...
| 互換性チェック | 新 version の `engine.chronos-fm_version` が範囲外なら update 不可、ユーザーに警告 |
| permission diff | 増えた permission のみ再プロンプト ([`docs/plugin-permissions.md`](./plugin-permissions.md) §2.1 参照) |
| ロールバック | 1 つ前のバージョンを `$DATA/chronos-fm/plugins/<id>/.prev/` に保持、UI でロールバック可 |
...
| 観点 | 仕様 |
|------|------|
| 完全削除 | config から除外、`$DATA/chronos-fm/plugins/<id>/` 削除、SQLite `plugins` レコード削除 |
...
```bash
# install
chronos-fm plugin install user/repo
chronos-fm plugin install https://gitlab.com/.../.git
chronos-fm plugin install ./local-plugin              # dev mode のみ

# update
chronos-fm plugin update                              # 全 plugin の更新確認
chronos-fm plugin update user/repo                    # 個別

# uninstall
chronos-fm plugin uninstall user/repo
chronos-fm plugin uninstall user/repo --purge         # KV データも削除

# list
chronos-fm plugin list

# Plugin Store への submit 用 PR 作成 (P5)
chronos-fm plugin publish                             # gh CLI 経由で web リポジトリに PR
```
...
- chronos-fm 側で blocklist を fetch (`https://chronos-fm.app/security/blocklist.json`)
- blocklist にある plugin は起動時に **強制 disable** + ユーザー通知
- blocklist は web リポジトリで管理、PR で追加

### 9.2 脆弱性報告

- `SECURITY.md` で `security@chronos-fm.app` (Future)、当面 GitHub Security Advisories
