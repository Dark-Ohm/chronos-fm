...
>
> **Работа принята. Одна поправка к разделу «Верификация».**
>
> Формулировка «БЛОКЕР (предсуществующий, не из T016) … Нужен фикс в форке
> gpui … вне зоны T016» неверна в части атрибуции. Конфликт `ashpd` внесён
> коммитом `07bce8b` (T011 Tasks 5+6), добавившим `oo7 = "0.6"` с default
> features: `oo7/tokio → ashpd/tokio` против `ashpd/async-io` из
> `gpui_linux`. `git log -S oo7 -- crates/chronos-fm-core/Cargo.toml` даёт
> единственный коммит; форк ни при чём. Ту же ошибку независимо повторили
> T013 и T014-recon. Исправлено `0564c6e`.
>
> Правки в `app.rs` и `search_bar.rs`, которые из-за блокера «проверены по
> коду, но не собраны», теперь собраны и проверены живьём: в логе запуска
> релизного бинаря `Failed to initialize search service` — **0 вхождений**,
> индекс поднялся (`Index already has 2750 documents`). Строка деградации
> `⚠ Full-text search unavailable` на месте (`search_bar.rs:218`).
>
> Вердикт приёмки: **VERIFIED WITH CAVEATS** — сам фикс подтверждён живьём,
> оговорка касается только атрибуции блокера.
