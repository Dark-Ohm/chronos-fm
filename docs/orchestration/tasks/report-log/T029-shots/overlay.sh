...
set -u
S=/tmp/claude-1000/-home-neo-projects-chronos-ecosystem-Source/68c3db57-8a34-40c2-bf0a-4a6b84ae2efc/scratchpad/t029
...
for a in "$@"; do
  ...
  case "$verb" in
    move|click|rclick)
      ...
      # "r<offset>" = offset from the RIGHT edge. Toasts are pinned top-right, so
      # their close button moves with window width (T029: width changed 1086->675
      # between launches).
      if [ "${rx#r}" != "$rx" ]; then rx=$(( w + ${rx#r} )); fi
      cx=$(( ax + rx )); cy=$(( ay + ry ))
      ...
      ;;
    sleep) sleep "$arg" ;;
    # T029: dismissal needs THREE capture points (before / opened / closed), so an
    # explicit mid-run shot. Стена 3: "closed" is proven by the frame matching
    # --before, not by the absence of an error.
    shot) grim -g "$ax,$ay ${w}x${h}" "$S/shots/$out--$arg.png" ;;
    # key:1 = Esc (ydotool uses Linux keycodes; 1 == KEY_ESC). Press+release.
    key) ydotool key "${arg}:1" "${arg}:0"; sleep 0.4 ;;
  esac
done
