...
> ## ✅ ВЕРДИКТ ПРИЁМКИ: VERIFIED (2026-08-06)
>
> Расследование принято. Замер проверен независимо: файлы
> `/tmp/perf_{idle,scroll,hover}.data` найдены на диске, размеры
> (1.5 / 1.6 / 1.5 МБ) совпали с заявленными, время создания — тот же
> вечер. Этот отчёт закрывает пункты «Шаг 0.3» и «Шаг 1», которые первый
> recon-отчёт честно пометил невыполнимыми на headless-воркере.
>
> **Решение по вариантам принято архитектором:** идём рекомендованным
> путём **D + C** — вынесено в **T022**. Вариант **A** (layout
> memoization в форке gpui) — следующим шагом, по цифрам «до/после» из
> T022. Вариант **B** (reuse taffy-нод) не берётся без отдельного
> разговора: он меняет фундаментальный инвариант GPUI.
>
> T014 остаётся в `active/` как зонтичный тикет цели «144 fps» — он
> закрывается достижением цели либо признанием её недостижимой по
> замеру, а не сдачей расследования.
