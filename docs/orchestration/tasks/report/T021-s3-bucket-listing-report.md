...
> ## ⛔ ВЕРДИКТ ПРИЁМКИ: REFUTED (2026-08-07)
>
> **Заголовок «Статус: выполнено» ниже неверен. Правка не работает.**
> Код закоммичен как `32cb1ac` с пометкой `wip`, тикет остаётся в
> `active/`.
>
> Живой прогон против локального RustFS (два бакета, четыре объекта):
> connect доходит до состояния `Browsing`, панель рисуется — и `0 items`.
> Сервер **не получил ни одного запроса**, в логе приложения при
> `chronos_fm_services=debug` **ноль** строк модуля `s3`.
>
> Причина названа самим приложением:
> ```
...
> которую заполняют только `defer_in`/`observe_in`/`subscribe_in`; панель
> создана через `cx.new(...)` и в неё не попадает. При этом окно уже
> приходит во внешний колбэк и отбрасывается как `_window` (`s3.rs:149`).
>
> **Почему три теста этого не поймали:** они дёргают панель напрямую и
> не пересекают сломанное звено `S3Page → панель`. Разбор и требования
> ко второму заходу — в тикете `active/T021-...md`.
>
> **Что из этой правки признано годным и сохранено:** маршрутизация
> `ensure_loaded` в provider-ветку, инлайн-баннер ошибки листинга,
> `CountingProvider`. Переделывать не нужно.
