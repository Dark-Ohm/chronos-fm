...
>
> Approach A реализован. Подтверждено: мёртвый блок
> `Home/Favorites/Recent/Trash` удалён, дубля `Home` нет, `get_shortcuts()`
> вынесен из рендера в `compute_shortcuts()`.
>
> **Таблица §2 «Верификация» содержит галочку, полученную чтением кода
> при непроведённом живом прогоне,** и живой прогон её опроверг:
>
> > «Иконки действий не упираются в край ✅»
>
> Иконок не было **вообще** — `icons/hard-drive.svg` и
> `icons/arrow-up.svg` не существовали в `assets/icons/`, и приложение
> писало по две строки `ERROR could not find asset` **на каждый кадр**.
> Кнопка eject была невидима. Заведено **T019**, исправлено и
> подтверждено живьём (коммит `18ab839`): лог чист, обе иконки видны,
> пути устройств получили эллипсис.
>
> Побочно T019 нашёл третий отсутствующий ассет, `info.svg`, который
> смок не заметил — иконка рисуется только при наличии `status_message`.
>
> **Осталось незакрытым (пункт 3 тикета):** карточка Places обрывается на
> середине сайдбара, ниже пустота — тот самый дисбаланс ритма, ради
> которого тикет заводился. Единственный открытый пункт из шести.
>
> Тикет остаётся в `active/`.
