...
本書は chronos-fm の開発環境セットアップを定めます。OS と用途に応じた推奨セットアップを示します。
...
| **Linux + Docker (headless / CI)** | `docker compose -f docker/ci/docker-compose.yml run --rm chronos-fm <cmd>` (Xvnc) |
...
```bash
# GPU あり（cargo test）
docker compose -f docker/ci/docker-compose.gpu.yml run --rm chronos-fm cargo test

# GPU なし（cargo test）
docker compose -f docker/ci/docker-compose.yml run --rm chronos-fm cargo test

# スクリーンショット
docker compose -f docker/ci/docker-compose.gpu.yml run --rm \
  chronos-fm bash -c "cargo build -p chronos-fm && script/ui-run.sh shot /chronos-fm/screenshot.png"
```
...
```bash
git clone https://github.com/chronos-fm/chronos-fm.git
cd chronos-fm
# 環境別に上記セクションのいずれか
```
