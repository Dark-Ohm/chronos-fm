...
[`crates/chronos-fm-pages/src/explorer/tests.rs`](../crates/chronos-fm-pages/src/explorer/tests.rs)。
...
|------------|--------|----------|------|------|
| **coverage-core** | `ubuntu-latest` | `cargo llvm-cov -p chronos-fm-core --locked --cobertura ...` | 基盤 crate `chronos-fm-core` | `--fail-under-lines 80` |
...
- 「core」は基盤 crate `chronos-fm-core` を指す。`chronos-fm-models` / `chronos-fm-services` は overall tier
...
|------|------|------|
| core (`chronos-fm-core`) | line ≥ **80%** | 各ファイル ≥ **80%** (全 core ファイルが対象) |
...
- gpui バイナリのエントリポイント (`chronos-fm/src/{app,cli,main}.rs`)
...
- 外部プロセス / 索引依存の検索バックエンド (`chronos-fm-services/src/search*`)
...
- **`[bans]`** — `multiple-versions = "warn"` / `openssl-sys` を deny。`tokio` の ban は**まだ設置できない**: ① `chronos-fm-services` が直接依存（ADR 0004 / P2 で撤去予定）、② gpui が `zed-reqwest → hyper → h2 → tokio-util → tokio` を transitively 引き、`wrappers` で表現できない。両方が解消するまで hard ban は CI を即落とすため保留（deny.toml にコメントで明記）。
...
| crate | ベンチ対象 |
|-------|-----------|
| `chronos-fm-store` | SQLite/redb の get/put レイテンシ、batch 操作 |
| `chronos-fm-services` | fs listing 並列度、search クエリ実行 |
| `chronos-fm-launcher` (P3) | nucleo ranking、起動時間 |
| `chronos-fm-plugin-host` (P4) | WIT host call レイテンシ、permission check overhead |
