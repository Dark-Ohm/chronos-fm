...
Chronos-FM は README 上 macOS 向けだが、GUI フレームワーク gpui は Linux にも対応している。
...
GUI は `gui` フィーチャー付きの `chronos-fm` バイナリ。手順 1 の symlink ディレクトリを
`RUSTFLAGS` でリンカに渡す。

```bash
RUSTFLAGS="-L $HOME/.local/devlibs" cargo build --features gui --bin chronos-fm
```

成功すると `target/debug/chronos-fm` が生成される。
...
```bash
DISPLAY=:108 target/debug/chronos-fm
```
...
```bash
DISPLAY=:108 setsid target/debug/chronos-fm > /tmp/chronos-fm.log 2>&1 < /dev/null &
# 描画開始までログ上のマーカーを待つ
timeout 20 bash -c 'until grep -q "Refreshing every" /tmp/chronos-fm.log; do :; done'
perl -e 'select(undef,undef,undef,4)'   # UI 描画の落ち着き待ち（sleep 代替）
DISPLAY=:108 xwd -root -silent -out /tmp/chronos-fm.xwd
python3 xwd2png.py /tmp/chronos-fm.xwd /tmp/chronos-fm.png
```
...
| `pkill -f target/debug/chronos-fm` で操作中シェルごと落ちる | `-f` がそのパス文字列を含む自分のシェルにマッチするため。**`pkill -x chronos-fm`（プロセス名で厳密一致）を使う** |
