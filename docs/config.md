...
| データ種別 | 場所 |
|-----------|------|
| **config** | `$XDG_CONFIG_HOME/chronos-fm/config.toml` (Linux/macOS: `~/.config/chronos-fm/config.toml`) |
| **SQLite DB** (メタデータ・履歴) | `$XDG_DATA_HOME/chronos-fm/db.sqlite` |
| **redb (ホスト KV, P2)** | `$XDG_DATA_HOME/chronos-fm/state.redb` |
| **Tantivy index** | `$XDG_DATA_HOME/chronos-fm/index/` |
| **redb (plugin KV, P4)** | `$XDG_DATA_HOME/chronos-fm/plugin-kv.redb` |
| **ログ / キャッシュ** | `$XDG_CACHE_HOME/chronos-fm/` |
| **plugin インストール先** | `$XDG_DATA_HOME/chronos-fm/plugins/<plugin-id>/` |
...
# 出力は tracing 経由 (target = "chronos_fm_store::sql" / "chronos_fm_store::redb")、RUST_LOG で絞り込み可。
...
| 中 | 設定ファイル | `~/.config/chronos-fm/config.toml` |
| 高 | 環境変数 | `CHRONOS_FM_THEME=dark` |
...
```bash
# GUI バイナリ経由 (macOS):
cargo run --bin chronos-fm -- config schema > docs/config.schema.json

# GUI 非依存 (Linux/CI、上と同一バイト列を出力):
cargo run -p chronos-fm-core --example schema > docs/config.schema.json
```

**CI で常に最新を保証**: `config schema` ジョブが上記 example でスキーマを再生成し、コミット済みファイルと `git diff --exit-code` する。構造体を変更してスキーマ再生成を忘れると、diff 付きで CI が落ちる。加えて `chronos-fm-core` のユニットテスト (`committed_schema_is_up_to_date`) が同じ検証をローカル/`cargo test` でも行う。

config.toml の冒頭に:

```toml
#:schema https://chronos-fm.app/schema/config.schema.json
```
...
**実装メモ**: P1 で `notify` を最小利用 (`chronos-fm-core::config::watcher`)。検索系の watcher は P3 で復活するが、config 監視は別パスで先行。
...
| **未知の `schema_version`** | 起動時に「chronos-fm の新しいバージョンが必要かもしれません」を表示、デフォルト config にフォールバック |
...
```bash
chronos-fm config show          # 現在の merged config を JSON で表示
chronos-fm config edit          # $EDITOR で config.toml を開く
chronos-fm config validate      # schema 検証
chronos-fm config schema        # JSON Schema を stdout に
chronos-fm config path          # config.toml のフルパスを表示
chronos-fm config reset         # config を初期化 (backup を取る)
```
