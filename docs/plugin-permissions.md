...
wasmtime-wasi は tokio 依存 ([ADR 0004](./adr/0004-remove-tokio.md) §6)。これを **プロセス共有のグローバル runtime にせず、`chronos-fm-plugin-host` 内の専用 `current_thread` runtime に閉じる** のは依存衛生だけが理由ではなく、**可用性の隔離** でもある:
...
- `~/dev/chronos-fm-plugins/<path>` を `[plugins.dev_paths]` に登録、permission prompt なしで load
...
| **fs.write: システムパス** | `/etc/`, `/usr/`, `/bin/`, `/sbin/`, `~/Library/Preferences/`, `~/Library/Application Support/` (chronos-fm 自身を除く) | manifest で要求しても install 時に hard reject |
| **fs.write: chronos-fm データ** | `$XDG_DATA_HOME/chronos-fm/db.sqlite`, `$XDG_DATA_HOME/chronos-fm/state.redb`, `$XDG_DATA_HOME/chronos-fm/plugin-kv.redb` | 他 plugin のデータや host state の破壊を防ぐ |
