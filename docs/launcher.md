...
```rust
// crates/chronos-fm-launcher/src/command.rs
...
`inventory` クレートで linker-time にコマンドを集める。各 crate (`chronos-fm-pages`, `chronos-fm-services`, `chronos-fm-launcher` 自体) が自身のコマンドを `inventory::submit!` で宣言。
