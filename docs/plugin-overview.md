...
```text
                    ┌────────────────────────────────┐
                    │       chronos-fm (host process)      │
                    │                                 │
                    │  ┌──────────────────────────┐  │
                    │  │  chronos-fm-plugin-host       │  │
                    │  │  - wasmtime-wasi + tokio │  │
                    │  │  - WIT host imports      │  │
                    │  │  - permission guard      │  │
                    │  └────┬─────────┬───────────┘  │
...
| tokio 隔離 | wasmtime-wasi は tokio 依存。`chronos-fm-plugin-host` 内の専用 `current_thread` runtime に閉じ込め、アプリコアは tokio-free を維持 ([ADR 0004](./adr/0004-remove-tokio.md)) |
...
- 重い計算
- chronos-fm リポジトリに同梱 (`crates/plugins/chronos-fm-plugin-<name>`)
...
- インストール先: `$XDG_DATA_HOME/chronos-fm/plugins/<plugin-id>/`
  ```toml
  [plugins]
...
| 配布 | chronos-fm リポジトリ同梱 | 個別 install |
| update | chronos-fm アップデートに同期 | 個別 update |
...
| chronos-fm version 範囲 | `engine.chronos-fm_version` で minimal 指定 |
| WIT world version | plugin は特定 WIT world (`chronos-fm:plugin@0.1.0` 等) に依存、host 側で複数 world バージョンをサポート (例: `0.1.x` と `0.2.x` 両対応) |
...
AI agent 開発支援 (`.factory/skills/`, MCP server `@chronos-fm/mcp-plugin-dev`) は各テンプレに同梱。
