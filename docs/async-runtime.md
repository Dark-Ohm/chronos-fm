...
| **HTTP server を持たない** | chronos-fm は GUI app であり axum/HTTP server は当面不要 |
...
wasmtime core の async は executor 非依存だが、**wasmtime-wasi は tokio に深く結合している** (公式に「Tokio executor を必要とし、設計に結びついている」と明記)。chronos-fm プラグインは WASI Preview 2 を前提とする ([ADR 0005](./adr/0005-wit-bindgen-component-model.md)) ため、ホストは wasmtime-wasi を使い、tokio が必要になる。

方針: **アプリ全体を tokio 化せず、tokio を `chronos-fm-plugin-host` 内の専用 `current_thread` runtime に隔離する**。
...
将来の `chronos-fm-gpui-wasmtime` (GPUI executor 上で wasmtime async を駆動し、専用 tokio runtime を不要にするブリッジ) は Future Work。詳細は [`ROADMAP.md`](./ROADMAP.md) §Future Work。
...
| **検証** | `cargo-deny` で tokio を deny し、`chronos-fm-plugin-host` 経由 (P4 以降) のみ `wrappers` で許可。P2〜P3 は plugin host 未導入のため `cargo tree \| grep -E '^tokio'` は **空** |
...
[bans]
deny = [
  # tokio はアプリコアでは禁止。WASI プラグイン実行層 (chronos-fm-plugin-host, P4 以降)
  # からの依存のみ wrappers で許可する
  { name = "tokio", wrappers = ["chronos-fm-plugin-host"] },
  { name = "tokio-util", wrappers = ["chronos-fm-plugin-host"] },
  { name = "openssl-sys" },        # rustls 統一
]
```

`chronos-fm-plugin-host` 以外の crate に tokio が混入したら CI で fail。P2〜P3 は plugin host 未導入のため tokio は完全に消える。
