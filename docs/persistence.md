...
| ファイル | 用途 | フェーズ |
|---------|------|---------|
| `$XDG_DATA_HOME/chronos-fm/state.redb` | ホスト KV (window 位置・タブ/セッション復元・動的設定) | **P2** |
| `$XDG_DATA_HOME/chronos-fm/plugin-kv.redb` | プラグイン専用 KV / cache | P4 |

### ホスト KV テーブル設計 (P2)

```rust
// crates/chronos-fm-store/src/chronos_fm_store.rs (擬似コード)
...
```rust
// crates/chronos-fm-store/src/chronos_fm_store.rs
...
ストア操作の所要時間を計測してパフォーマンス解析に使うためのログ機構。デフォルトは全 off (本番は無音) で、`config.toml` の `[diagnostics.store]` で有効化する (スキーマは [`docs/config.md`](./config.md) §2)。出力は既存の `tracing` + `EnvFilter` (`RUST_LOG`) 経路 ([`crates/chronos-fm-core/src/telemetry/logging.rs`](../crates/chronos-fm-core/src/telemetry/logging.rs)) にそのまま乗る。

### SQLite

rusqlite の組み込みフックを使う:

- `Connection::profile(Some(callback))` — 各ステートメント完了後に実行 SQL と所要時間を受け取る。`slow_query_ms` 超過なら `warn`、`log_all_queries` 有効時は全件を `debug` で `tracing` へ emit (target = `chronos_fm_store::sql`)。
- `Connection::trace(Some(callback))` — 必要なら展開後 SQL を `trace` レベルで出力 (より詳細)。

### redb

redb には同等の組み込みフックが無いため、`RedbKvStore` の各操作 (`get` / `put` / `delete` / `batch`) を計測ラッパで囲み、`log_redb_ops` 有効時に操作名と所要時間を `tracing` へ emit (target = `chronos_fm_store::redb`)。
...
- いずれも単一ファイル。P2 では `db.sqlite` (メタデータ・履歴) と `state.redb` (ホスト KV) の 2 ファイル、P4 で `plugin-kv.redb` が加わる
  ```sh
  cp ~/.local/share/chronos-fm/db.sqlite   ./backup-$(date +%Y%m%d).sqlite
  cp ~/.local/share/chronos-fm/state.redb  ./backup-$(date +%Y%m%d).redb
  ```
