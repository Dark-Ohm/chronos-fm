...
- **Never `pkill -f chronos-fm`** — the pattern matches this script's own path / your shell and kills
  it (exit ~143/144). Use `pkill -x chronos-fm` (exact process name). `ui-run.sh stop` does this.
