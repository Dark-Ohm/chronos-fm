...
本書は chronos-fm の Cargo workspace 構成、crate 間の依存方向、レイヤーごとの責務、主要なデータフローを定めます。
...
├── rust-toolchain.toml
├── crates/
│   ├── chronos-fm/                # main binary (P1)
│   ├── chronos-fm-core/           # errors / config / telemetry (P1)
│   ├── chronos-fm-models/         # FileEntry など pure data types (P1)
│   ├── chronos-fm-services/       # fs / search / syntax (P1)
│   ├── chronos-fm-ui/             # gpui components / theme / app shell (P1)
│   ├── chronos-fm-pages/          # explorer / settings / git / s3 (P1)
│   ├── chronos-fm-store/          # SQLite + redb + MetadataStore trait (P2)
│   ├── chronos-fm-launcher/       # launcher (P3)
│   ├── chronos-fm-plugin-host/    # wasmtime + WIT host (P4)
│   └── plugins/              # コアプラグイン群 (P4 以降に追加)
│       ├── chronos-fm-plugin-git/
│       └── ...
...
```text
                 ┌──────────────┐
                 │  chronos-fm (bin) │   GUI エントリポイント + CLI
                 └──────┬───────┘
                        │
        ┌───────────────┼───────────────┐
        ▼               ▼               ▼
...
| crate | 主な責務 | 依存 |
|-------|---------|------|
| **chronos-fm (bin)** | GUI エントリ、CLI サブコマンド、起動シーケンス | 全 crate |
| **chronos-fm-pages** | explorer / settings / git / s3 などのページ | `ui`, `services`, `models`, `core`, (P2) `store` |
| **chronos-fm-launcher** (P3) | ランチャー window、Command trait、結果リスト | `ui`, `services`, `core`, `store`, (P4) `plugin-host` |
| **chronos-fm-plugin-host** (P4) | wasmtime + wasmtime-wasi + WIT host、permission ガード、専用 tokio runtime (隔離) | `core`, `store`, `services`, `models` |
| **chronos-fm-ui** | gpui コンポーネント、テーマ、ウィンドウ管理 | `core`, `models` |
| **chronos-fm-services** | fs listing、search、syntax highlighting | `core`, `models`, (P2) `store` |
| **chronos-fm-store** (P2) | SQLite (rusqlite) + redb (plugin KV) | `core`, `models` |
| **chronos-fm-models** | FileEntry など pure data types | `core` |
| **chronos-fm-core** | errors / config / telemetry / resource policy | — |
...
- **本体 crate**: `chronos-fm-<role>` (例: `chronos-fm-core`, `chronos-fm-services`)
- **コアプラグイン**: `chronos-fm-plugin-<name>` (例: `chronos-fm-plugin-git`、`crates/plugins/` 配下)
- **公開ライブラリ (Future Work)**: 別リポジトリで独立、prefix は文脈に応じて (例: `chronos-fm/gpui-wasmtime`)

`chronos-fm-` prefix を統一する理由: crates.io 公開時の名前衝突回避と、依存ツリーで一目で chronos-fm 由来と分かるため。
...
| `bans` (cargo-deny) | `tokio` を deny (P2 以降)。`chronos-fm-plugin-host` 経由のみ `wrappers` で許可 | plugin 実行層 (P4) |
...
- `src/lib.rs` のような generic 名は避け、`src/chronos_fm_core.rs` のように記述

---

## 7. 移行計画 (P1)

現状の単一 crate `chronos-fm` (約 5000 行) を 6 crate に分割するロードマップ:

1. `crates/chronos-fm-core/` を切り出し (`src/core/` → `crates/chronos-fm-core/src/chronos_fm_core.rs`)
2. `crates/chronos-fm-models/` を切り出し
3. `crates/chronos-fm-services/` を切り出し
4. `crates/chronos-fm-ui/` を切り出し
5. `crates/chronos-fm-pages/` を切り出し
6. `crates/chronos-fm/` (binary) を整理、`src/gui/main.rs` を `crates/chronos-fm/src/main.rs` へ
